...
var Str = "null "
var Pos
var B_Matrix = 	new Array(8)
var D_Matrix = new Array(8)
for (var i=0; i<8; i++){
	B_Matrix[i] = [0,0,0,0,0,0,0,0]
	D_Matrix[i] = [0,0,0,0,0,0,0,0]
}	
...
function bang() {
	...
	var tempcolor = 51
	displayChar(Str.charAt(Pos[0]),0,Pos[1],tempcolor)
	if (Pos[1]!=0) {
		displayChar(Str.charAt(Pos[0]+1),0,Pos[1]-8,tempcolor)
	}
...
function displayChar(charactor,row,column,color) {
	var code
	switch (charactor) {
		case " ": code = 0; break;
		case "!": code = 1; break;
	...
	var fontU = fontdata[code*2]
	var fontD = fontdata[code*2+1]
	...
	for (var r=0; r<8; r++){
		for (var c=0; c<8; c++){
			...
			if (0<=r2 && r2<=7 && 0<=c2 && c2<=7) {
				//double buffering
				D_Matrix[c2][r2] = fmatrix[c][r] * color;
			}
		}
	}
}
